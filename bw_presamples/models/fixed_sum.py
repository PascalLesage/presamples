...
class FixedSum(ModelBase):
    ...
    def run(self):
        self.array = np.vstack([self.random_sample(o) for o in self.data])
        scale = ((self.expected - self.array[~self.mask, :].sum(axis=0)) /
                 self.array[self.mask, :].sum(axis=0))
        self.array[self.mask, :] *= scale
        return self.array
