...
def test_kronecker_basic():
    ...
    k.run()
    assert isinstance(k.array, np.ndarray)
    assert k.array.shape == ((3, 1000))
    sums = k.array.sum(axis=1)
    assert sums[0] < sums[1] < sums[2]
    assert set(np.unique(k.array)) == {0, 1}
    assert np.allclose(k.array.sum(axis=0), np.ones((1000,)))
...
def test_kronecker_no_normalize():
    ...
    sums = k.array.sum(axis=1)
    assert sums[0] < sums[1] < sums[2]
    assert set(np.unique(k.array)) == {0, 1, 3, 6}
    assert set(np.unique(k.array[1, :])) == {0, 3}

def test_kronecker_equal_choice():
    k =  KroneckerDelta([{'amount': 1}, {'amount': 3}, {'amount': 6},], iterations=10000, equal_choice=True)
    k.run()
    assert isinstance(k.array, np.ndarray)
    assert k.array.shape == ((3, 10000))
    sums = k.array.sum(axis=1)
    for x in range(3):
        assert 2500 < sums[x] < 4000
    assert set(np.unique(k.array)) == {0, 1}
    assert np.allclose(k.array.sum(axis=0), np.ones((10000,)))
...
def test_kronecker_negative():
    ...
    assert set(np.unique(k.array)) == {0, 1, -1}
    assert set(np.unique(k.array[1, :])) == {0, -1}
